...
public class Cafe {
	...
	// 메서드(Method)부
	/*
	 * 기능(method)를 구현하는 부분(대상의 기능들) VO는 값과 관련된 메서드가 들어가야 함 접근제한자 (예약어(static 있을수도 있고
	 * 없을수도 있고)) 반황형 메소드식별자(매개변수(파라미트)) { 메소드 호출 시 실행할코드; }
	 * 
	 * 접근제한자 : 호출할 수 있는 범위를 제한할 수 있음 반환형 : void => return 할 때 돌려줄 값이 없음 반환 시 결과 값의
	 ...
	 * setter만들기 규칙
	 * 
	 * 1. setter메소드는 외부에서 접근이 가는해야 하기 때문에 접근제한자 public을 이용 2. set필드명으로 식별자를 작성하며
